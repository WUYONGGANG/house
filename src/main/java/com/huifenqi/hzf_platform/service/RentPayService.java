...
public class RentPayService {
	...
	public JsonObject rentWithhold(String userId, String rentContractCode, int isUseWithhold, String bankToken,
	   int isUseCA,String signatureUrl ) {
		...
		params.put("isUseCA",  String.valueOf(isUseCA));
		...
		if(isUseCA==1) {
			if(StringUtils.isNotBlank(signatureUrl)) {
				...
				throw new BaseException(ErrorMsgCode.ERRCODE_INTERNAL_ERROR,"signatureUrl 不能为空");
			}
		}
	 ...
	 * @dateTime 2017年11月22日 下午4:48:18
	 */
	public JsonObject reqElectricalContract(String userId,String signatureUrl,String rentContractCode) {
		String userServiceUrl = configuration.hfqHzfSassServiceHost + "/v3/hzf/contractsignature/";
		Map<String, String> params = new HashMap<>();
		params.put("userId", userId);
		params.put("rentContractCode", rentContractCode);
		params.put("signatureUrl", signatureUrl);
	...
	/**
	 * @Title: reqDepositBugetDetail
	 * @Description: 获取交定账单详情
	 * @return JsonObject
	 * @author 叶东明
	 * @dateTime 2018年3月21日 下午2:17:48
	 */
	public JsonObject reqDepositBugetDetail(long userId, String bugetNo) {
        String userServiceUrl = configuration.hfqHzfSassServiceHost + "/v3/hzf/reqdepositbugetdetail/";
        Map<String, String> params = new HashMap<>();
        params.put("userId", String.valueOf(userId));
        params.put("bugetNo", bugetNo);
        return requestRemote(userServiceUrl, params);
	 ...
	 * @return JsonObject
	 * @author 叶东明
	 * @dateTime 2018年3月1日 下午5:45:30
	 */
	public JsonObject reqBugetPaidDetail(long userId, String bugetNo) throws Exception {
        String userServiceUrl = configuration.hfqHzfSassServiceHost + "/v3/hzf/bugetpaydetail/";
        Map<String, String> params = new HashMap<>();
        params.put("userId", userId+"");
        params.put("bugetNo", bugetNo);
	 ...
	 * @return JsonObject
	 * @author 叶东明
	 * @dateTime 2018年3月16日 下午3:11:18
	 */
	public JsonObject reqContractConfirm(long userId, String contractNo) {
        String userServiceUrl = configuration.hfqHzfSassServiceHost + "/v3/hzf/reqcontractconfirm/";
        Map<String, String> params = new HashMap<>();
        params.put("userId", String.valueOf(userId));
        params.put("rentContractCode", contractNo);
        return requestRemote(userServiceUrl, params);
    }
	
	/**
	 * @Title: reqDepositBugets
	 * @Description: 获取交定账单数据
	 * @return JsonObject
	 * @author 叶东明
	 * @dateTime 2018年3月20日 上午11:25:22
	 */
	public JsonObject reqDepositBugets(long userId, String phone) {
        String userServiceUrl = configuration.hfqHzfSassServiceHost + "/v3/hzf/reqdepositbugets/";
        Map<String, String> params = new HashMap<>();
        params.put("userId", String.valueOf(userId));
        params.put("phone", phone);
        return requestRemote(userServiceUrl, params);
    ...
    public JsonObject requestRemote(String url,Map<String, String> params ) {
        ...
        ApiResult result = GsonUtil.buildGson().fromJson(postRet, ApiResult.class);
        String code = result.status.code;
        if("2620198".equals(code)){
			throw new ServiceInvokeFailException(Integer.parseInt(code), "该版本暂不支持签约功能，请更新APP后重试");
		}else if (!"0".equals(code)) {
            throw new ServiceInvokeFailException(Integer.parseInt(code), result.status.description);
        }
        return result.result;
