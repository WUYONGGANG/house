...
//	}
	...
	public static PrivateKey getPrivateKey(String privateKey) {
		try {
			byte[] keyBytes;
			keyBytes = AszBase64Util.decode(privateKey);
	...
	public static PublicKey getPublicKey(String publicKey) {
		try {
			byte[] keyBytes;
			keyBytes = AszBase64Util.decode(publicKey);
	...
	protected static byte[] decryptByPublicKey(String content, PublicKey pk) {
		...
		try {
			...
			ins = new ByteArrayInputStream(AszBase64Util.decode(content));
