...
import com.huifenqi.hzf_platform.context.dto.request.house.BaiduMapSysHouseDetailRequestDto;
import com.huifenqi.hzf_platform.context.dto.request.house.BaiduMapSysHouseDetailToLocalDto;
...
//            }
     ...
     * @return String url
     */
    public static Map<String, String>  getCommunityParam(String key,JsonArray jsonArray,CommunityBase c){
        Map<String, String> params = new HashMap<>();
        params.put("src", "huifenqi");
        BaiduMapSysHouseDetailRequestDto requestDto = new BaiduMapSysHouseDetailRequestDto();
        requestDto.setId(String.valueOf(c.getId()));//小区ID
        requestDto.setStatus(c.getBaiduMapType());//操作类型
        requestDto.setData(jsonArray);
        
        //请求转换为json字符串
        String input = GsonUtils.getInstace().toJson(requestDto);
        
        //json加密
        String requestData = AesSecurityUtil.encrypt(input, key);
