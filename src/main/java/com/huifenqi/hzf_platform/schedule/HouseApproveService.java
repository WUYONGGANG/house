...
import com.huifenqi.hzf_platform.dao.repository.house.BlackPhoneRepository;
...
public class HouseApproveService {
	...
	@Resource
    private BlackPhoneRepository blackPhoneRepository;
	...
	public void checkHouseErrorData(HouseDetail hd, HouseBase hb) {
		...
		//默认审核通过
        if(hd.getSource().equals("全房通") || hd.getSource().equals("蛋壳公寓")){
                result = false;
        }
        
        if(!hb.getAgencyPhone().isEmpty()){
            int rows = blackPhoneRepository.findBlackByPhone(hb.getAgencyPhone());
            if(rows > 0){
                result = true;
                reason.append(HouseDataErrorEnums.BLACK_PHONE.getDesc()).append(","); 
            }
        }
        
		if (result) {
			int rentType = hd.getEntireRent();// 0 合租，1 整租
			logger.info("houseSellId = {},rentType = {}, 房源错误信息：{}", hd.getSellId(), hd.getEntireRent(),reason.toString());
			...
			//默认房间审核通过
            if(hd.getSource().equals("全房通") || hd.getSource().equals("蛋壳公寓")){
                roomBaseRepository.setApproveStatusBySellId(hd.getSellId(), ApproveStatusEnum.SYS_APP_PASS.getCode());
            }
		}
