...
public class WeixinController {
	 ...
	 * @return Responses
	 * @author 叶东明
	 * @dateTime 2018年3月13日 下午6:14:18
	 */
	@RequestMapping(value = "/wx/reqwxqrcode/", method = RequestMethod.POST)
    public Responses reqWxQrCode(HttpServletRequest request, HttpServletResponse httpServletResponse) {
        return weixinRequestHandler.reqWxQrCode();
    }
	
	/**
	 * @Title: reqWxMsg
	 * @Description: 微信推送消息（合同）
	 * @return Responses
	 * @author 叶东明
	 * @dateTime 2018年3月14日 下午2:42:36
	 */
	@ResponseBody
	@RequestMapping(value = "/wx/reqwxmsg/")
    public void reqWxMsg(HttpServletResponse response, HttpServletRequest request, 
        ...
        if (!weiXinKits.checkSignature(signature, timestamp, nonce, token)) {
            logger.error("signature error");
            weixinRequestHandler.wxResponse(response, "signature error");
            return;
        }
        if (echostr != null) {
            weixinRequestHandler.wxResponse(response, echostr);
            return;
        }
        String reqMsg = weixinRequestHandler.reqWxMsg(request);

        logger.info("response:" + reqMsg);
        weixinRequestHandler.wxResponse(response, reqMsg);
        return;
    }
