...
public class RedisSaveHouseOptRecordWorker implements Runnable {
	...
	private void doWork() {
		String result = "";
		try {
			result = redisCacheManager.rightPop("hzf_platform-house.opt.record");
			logger.info("record保存历史queue剩余条数：" + redisCacheManager.sizeList("hzf_platform-house.opt.record"));
        ...
        try {
            ...
            if (dto != null) {
                ...
                if (houseDetail == null) {
                    ...
                    if(houseDetail.getIsDelete() == Constants.Common.STATE_IS_DELETE_NO){//房源未删除
                        if(houseDetail.getIsRun() == Constants.HouseBase.IS_RUN_NO){//房源未收集
                            savaFlag = true;
                        } 
                        
                          //查询未审核，未被删除，待出租，已收集       允许solr查 允许审核
              ...
              //4.查询房源信息
                ...
                if (dto.getRoomId() > 0) {
                    ...
                    if (roomBase == null) {
                        ...
                        if(roomBase.getIsDelete() == Constants.Common.STATE_IS_DELETE_NO){//房间未删除
