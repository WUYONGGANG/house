...
//	}
	...
	public Responses getPayChannelsV2() {
		...
		if (cardList != null) {
			...
			bankJsonObject.addProperty("name","银行卡支付(支付储蓄卡)");
			bankJsonObject.addProperty("type","bankCard");
			bankJsonObject.addProperty("desc","");
			JsonArray newArray = new JsonArray();
			Iterator<JsonElement> iterator = cardList.iterator();
			while (iterator.hasNext()) {
				JsonElement next = iterator.next();
				...
				if (next instanceof JsonObject) {
				    ...
				    bankJsonObject.add("list",GsonUtil.buildGson().toJsonTree(payChannelVos));
			}
			channels.add(bankJsonObject);
		}


		JsonObject wxJsonObject=new JsonObject();
		wxJsonObject.addProperty("name","微信支付");
		wxJsonObject.addProperty("type","wx");
		wxJsonObject.addProperty("desc","付费支付渠道，收取支付金额0.6%手续费");
		//jsonArray.add(wxJsonObject);
		JsonObject alipayObject=new JsonObject();
		alipayObject.addProperty("name","支付宝支付");
		alipayObject.addProperty("type","alipay");
		alipayObject.addProperty("desc","付费支付渠道，收取支付金额0.6%手续费");
		channels.add(alipayObject);
		JsonObject retJo = new JsonObject();
		retJo.add("channels", channels);
