...
public class RentContractRequestHandler extends BaseRequestHandler {
	...
	private List<RentContractVo> getRentContractVo(JsonObject jsonObject,long userId,String phone){
		...
		if(jsonObject.has("list")){
			...
			for(JsonElement je:jsonArray){
                ...
                // 合同确认引导
                int contractConfirm = jo.get("contractConfirm").isJsonNull()?0:jo.get("contractConfirm").getAsInt();
                rentContractVo.setContractConfirm(contractConfirm);
                rentContractVo.setContractConfirmName(Constants.contractConfirmName.get(contractConfirm));
                //新增分期合同业务
                int isApplied=jo.get("isFQ").isJsonNull()?0:jo.get("isFQ").getAsInt();
				rentContractVo.setIsApplied(isApplied);
                if(isApplied==1) {
					int status = jo.get("contractFQstatus").isJsonNull() ? 0 : jo.get("contractFQstatus").getAsInt();
					rentContractVo.setContractFQstatus(status);
					if (status == 1) {
						rentContractVo.setContractFQstatusName("月付办理中");
					}else if(status==2){
						rentContractVo.setContractFQstatusName("月付履约中");
					}
				}
	...
	private RentContractDetailVo getRentContractDetailVo(JsonObject jo,int needHidden,long userId,String phone,String rentContractCode){
		...
		//新加CA相关属性 不影响老版本
		int isUseCA =jo.get("isUseCA").isJsonNull()?0:jo.get("isUseCA").getAsInt();
		rentContractDetailVo.setIsUseCA(isUseCA);
		rentContractDetailVo.setElectronicContractUrl(jo.get("electronicContractHTMLURL").isJsonNull()?"":jo.get("electronicContractHTMLURL").getAsString());
		int isElectronicContrac=0; //普通合同
		if(!jo.get("isUseTemplate").isJsonNull( )&& jo.get("isUseTemplate").getAsInt()==1){
			isElectronicContrac=1; //是电子合同
		}
		rentContractDetailVo.setIsElectronicContract(isElectronicContrac);
		rentContractDetailVo.setPdfContractUrl(jo.get("electronicContractURL").isJsonNull()?"":jo.get("electronicContractURL").getAsString());

		//新增分期合同业务
        int isApplied=jo.get("isFQ").isJsonNull()?0:jo.get("isFQ").getAsInt();
        rentContractDetailVo.setIsApplied(isApplied);
        if(isApplied==1) {
            int status = jo.get("contractFQstatus").isJsonNull() ? 0 : jo.get("contractFQstatus").getAsInt();
            rentContractDetailVo.setContractFQstatus(status);
            if (status == 1) {
                rentContractDetailVo.setContractFQstatusName("月付办理中");
            }else if(status==2){
                rentContractDetailVo.setContractFQstatusName("月付履约中");
            }
        }
	...
	public Responses rentWithhold(HttpServletRequest request) throws Exception {
		...
		String rentContractCode= getStringParam("rentContractCode");
		int isUseCA=getDefaultIntParam("isUseCA",0);//枚举0 表示不使用ca
		String signatureUrl=getDefaultStringParam("signatureUrl","");

		JsonObject serviceRetJo = rentPayService.rentWithhold(userId+"",rentContractCode,isWithhold,bankToken,isUseCA,signatureUrl);
		Responses responses = new Responses();
		responses.getMeta().setErrorCode(ErrorMsgCode.ERROR_MSG_OK);
	...
	/**
	 * @Title: reqElectricalContract
	 * @Description: CA获取带签名电子合同接口
	 * @return Responses
	 * @author Arison
	 * @dateTime 2018年3月16日 上午11:42:15
	 */
	public Responses reqElectricalContract(HttpServletRequest request) throws Exception {
        long userId = sessionManager.getUserIdFromSession();
		String rentContractCode= getStringParam("rentContractCode");
		String signatureUrl= getStringParam("signatureUrl");
		JsonObject serviceRetJo = rentPayService.reqElectricalContract( userId+"",signatureUrl,rentContractCode);
		Responses responses = new Responses();
		responses.getMeta().setErrorCode(ErrorMsgCode.ERROR_MSG_OK);
		responses.setBody(serviceRetJo);
		return responses;
