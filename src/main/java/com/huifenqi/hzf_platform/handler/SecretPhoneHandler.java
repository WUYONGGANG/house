...
public class SecretPhoneHandler {
	...
	public String getUnbingSectetPhone(String sellId, String roomId, String agencyPhone) {
		...
		String secretNo = agencyPhone;//记录保护号
		
		String checkResultPhone = checkHistoryCallInfo(agencyPhone, sellId ,null);
		
		Integer assignStatus = SecretNoAssignStatus.ASSIGNED.getCode();//默认成功
		BindRecord record = new BindRecord();//绑定记录
		
		if (SecretPhoneUtil.checkTelPhone(checkResultPhone, false)) {//如果不是手机号，则返回原来的号码
			...
			if (!Objects.isNull(secret)) {
				...
				SecretBindDTO bindDto = secretPhoneService.bindAxn(checkResultPhone.replace("-", ""), secret.getSecretPhoneNo(), outId);
			...
			logger.error("此经纪人电话无法转接，agencyPhone={}", checkResultPhone);
		}

		record.setSellId(sellId);
		record.setRoomId(Long.parseLong(roomId));
		record.setAgencyPhone(checkResultPhone);
	 ...
	 * @return
	 */
	private String checkHistoryCallInfo(String agcPhone, String sellId, List<String> agencyPhoneList) {
		int totalCount = callInfoRecordRepository.selectAgencyPhoneCallInfoTotalCount(agcPhone);
		if (totalCount == 0) {
			return agcPhone;
		} else {
			int validCount = callInfoRecordRepository.selectAgencyPhoneCallInfoValidCount(agcPhone);
			if (validCount > 0) {
				return agcPhone;
			} else {
				if (Objects.isNull(agencyPhoneList)) {
					agencyPhoneList = callInfoRecordRepository.selectAgencyPhoneBySellId(sellId);
				}

				if (CollectionUtils.isEmpty(agencyPhoneList)) {
					return agcPhone;
				} else {
					Iterator<String> it = agencyPhoneList.iterator();
					while (it.hasNext()) {
						String agPhone = it.next();
						it.remove();
						String result = checkHistoryCallInfo(agPhone, sellId, agencyPhoneList);
