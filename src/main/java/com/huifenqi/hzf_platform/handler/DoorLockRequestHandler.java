...
import com.huifenqi.hzf_platform.context.enums.smartlock.GwModdityPwdInnerErrorCodeEnum;
...
public class DoorLockRequestHandler extends BaseRequestHandler {
	...
	public Responses getDoorLockList() throws BaseException {
		...
		lockParams.put("lockType", String.valueOf(LockTypeEnum.BLUETOOTH.getCode()));
	...
	/**
	 * @Title: getDoorLockList
	 * @Description: 获取智能门锁列表（升级）
	 * @return Responses
	 * @author Arison
	 * @dateTime 2017年12月19日 下午5:05:43
	 */
	public Responses getDoorLockListV2() throws BaseException {
		long userId =  sessionManager.getUserIdFromSession();
		int pageNum = getDefaultIntParam("pageNum", 1);
		int pageSize = getDefaultIntParam("pageSize", 20);
		Map<String, String> lockParams = new HashMap<>();
...
//			}
	...
	public Responses modifyPwd() throws BaseException {
		...
		if (OnlineEnum.OFFLINE.getCode().equals(isOnline)) {
			responses.getMeta().setErrorCode(GwModdityPwdInnerErrorCodeEnum.OFFLINE.getCode());
			responses.getMeta().setErrorMessage(GwModdityPwdInnerErrorCodeEnum.OFFLINE.getDesc());
			return responses;
		}
		if (!RulesVerifyUtil.verifyNoSixToTen(oldPwd)) {
			responses.getMeta().setErrorCode(GwModdityPwdInnerErrorCodeEnum.OLD_PWD_FORMAT_ERROR.getCode());
			responses.getMeta().setErrorMessage(GwModdityPwdInnerErrorCodeEnum.OLD_PWD_FORMAT_ERROR.getDesc());
			return responses;
		}
		if (!RulesVerifyUtil.verifyNoSixToTen(newPwd)) {
			responses.getMeta().setErrorCode(GwModdityPwdInnerErrorCodeEnum.NEW_PWD_FORMAT_ERROR.getCode());
			responses.getMeta().setErrorMessage(GwModdityPwdInnerErrorCodeEnum.NEW_PWD_FORMAT_ERROR.getDesc());
			return responses;
		}
		if (oldPwd.equals(newPwd)) {
			responses.getMeta().setErrorCode(GwModdityPwdInnerErrorCodeEnum.NOT_SAME.getCode());
			responses.getMeta().setErrorMessage(GwModdityPwdInnerErrorCodeEnum.NOT_SAME.getDesc());
			return responses;
		}
		Map<String, String> lockParams = new HashMap<>();
		lockParams.put("userId", String.valueOf(userId));
