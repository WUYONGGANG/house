...
public interface CommunityBaseRepository extends JpaRepository<CommunityBase, Long> {
	...
	@Query("select a from CommunityBase a" + " where a.baiduMapFlag=?1")
	public List<CommunityBase> findCommunityByBaiduMapFlag(int flag);
	
	@Modifying
	@Query("update  from CommunityBase a set a.baiduMapStatus=?1" + " where a.id=?2")
    public int updateCommunityBaseStatus(int baiduMapStatus,long id);
}
